    ...
    *reinterpret_cast<int*>((char*)nodeSet->at(i)+32)=0;
    ...
    if(_edgeIdxMask->get(i)) {//the idx is stored, case 1 not preliminary but filled<empty
      ...
      for (uint j=pIdx;j<pIdx+sz;++j) {
        nodeSet->at(i)->set_req(_edgeIdx[j],nodeSet->at(_edge[pEdge++]));
      }
      pIdx+=sz;
    }
      ...
      if (preliminary_known_node(nodeSet->at(i))) {//add sub...
        uint idx=0;
        if (sz==3)
          nodeSet->at(i)->set_req(idx++,nodeSet->at(_edge[pEdge++]));
        nodeSet->at(i)->set_req(idx,nodeSet->at(_edge[pEdge]));
        nodeSet->at(i)->set_req(idx+1,nodeSet->at(_edge[pEdge+1]));
      }
      else {//-1.
        for (uint j=0;j<sz;++j)
          if (_edge[j+pEdge]!=-1)
          nodeSet->at(i)->set_req(j,nodeSet->at(_edge[pEdge+j]));
      }
    }
  }
